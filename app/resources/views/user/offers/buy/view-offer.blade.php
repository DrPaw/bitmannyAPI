										...
										<div class="row no-gutter">
											<div class="col-lg-5 border-right pr-0">
												<div class="product-gallery p-4">
													...
													<br>
													Buyer Username: {{App\User::whereId($offer->user_id)->first()->username ?? "Unknown"}}
											...
											<div class="col-lg-7">
												<div class="product-gallery-data mb-0">
													...
													<dl class="product-gallery-data1">
														<dt>Amount</dt>
														<dd>${{number_format($offer->min,2)}}</dd>
													</dl>
													<dl class="product-gallery-data1">
														<dt>Buyer's Rate</dt>
														<dd>1USD = {{number_format($offer->rate,2)}}{{$offer->currency}}</dd>
													</dl>
													<dl class="product-gallery-data1">
														<dt>Trade Earning</dt>
														<dd>${{number_format($offer->min*$offer->rate,2)}}</dd>
													</dl>
													<dl class="product-gallery-data1">
														<dt>Country</dt>
														<dd>{{$offer->country}}</dd>
                                             ...
                                             <script>
                                                ...
                                                var usd = $('#usd').val();
                                                 var pay = usd*{{$offer->rate}};
                                                  document.getElementById("pay").innerHTML = "<b>You Get: "+pay+"{{$offer->currency}}</b>";

                                                };
                                                </script>
                                                    <form  id="form" class="contact-form" action="{{route('user.contactsellerbuy',$offer->code)}}" method="post" enctype="multipart/form-data">
                                                    @csrf
                                                    <small>Enter Amount To Sell (USD)</small><br>
													<div class="input-group">

													<input type="number"  id="usd" onkeyup="convert()" class="form-control" name="amount" max="{{$offer->max}}" placeholder="${{$offer->min}}" >
                                					</div>
                                					<a id="pay"></a>
                                					<!--<a id="window"></a>
                                					<br>
 ...
 var wallet = $("#wallet option:selected").attr('data-address');
  ...
  document.getElementById("address").innerHTML = "<b>Address: "+wallet+"</b>";
												...
												<select id="wallet" onchange="myFunction2()"  name="wallet" class="form-control select2-show-search custom-select">
													...
													<button type="submit"  style="background-color: {{$general->bclr}}" class="btn btn-primary mt-1"> <i class="fa fa-shopping-cart"></i> Contact Buyer </button>
													</form>
						...
						<div class="">
							<div class="card">
								...
								<div class="alert alert-info alert-dismissible fade show" role="alert">
                        			...
                        			<span class="alert-inner--text"><strong>Hello Seller!</strong> There is no comment or rating on this offer yet. Be the first to rate this buyer by proceeding with initiating a transaction on this offer!</span>
