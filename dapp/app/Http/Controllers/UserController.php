...
class UserController extends Controller
{
      ...
      public function hodl()
    {
        ...
        $data['page_title'] = "Hodl Plans";
        $data['plans'] = Hodl::where('status', 1)->latest()->get();
        $data['wallets'] = UserWallet::where('user_id', Auth::id())->get();
        return view('user.hodl.plan', $data);
    }

    public function buyhodl(Request $request)
    {
        ...
        $plan = Hodl::where('id', $request->plan_id)->where('status', 1)->first();
        if (!$plan) {
        ...
        if ($plan->fixed_amount == 0) {
            ...
            if ($plan->fixed_amount == $request->amount) {
                ...
                $user->save();
                $notify[] = ['success', 'Hodl Package Purchased Successfully Complete'];
                return redirect()->route('user.interest.log')->withNotify($notify);
            }
    ...
    public function hodlLog()
    {
        $page_title = 'HODL Log';
        $trans = Hodltrx::where('user_id', Auth::id())->latest()->get();
        return view('user.hodl.hodl_log', compact('page_title', 'trans'));
    }
