 ...
 public function searchp2p(Request $request)
    {
        ...
        $count = Cryptoffer::whereCoin_id($request->currency)->where('type', 1)->where('status', 1)->orderBy('id','desc')->count();
       if($count < 1){
        ...
        $data['offer'] = Cryptoffer::whereCoin_id($request->currency)->where('status', 1)->whereType(1)->whereCountry($request->country)->orderBy('id','desc')->get();
     ...
     public function viewoffer($id)
    {
        ...
        $data['offer'] = Cryptoffer::whereCode($id)->whereType(1)->where('status', 1)->orderBy('id','desc')->first();
        if(! $data['offer']){
        ...
        $data['bought'] = Cryptotrade::whereBuyer(Auth::id())->where('marketcode', $id)->wherePaid(1)->orderBy('id','desc')->count();
       ...
       public function rateseller(Request $request, $id)
    {
        ...
        $trade = Cryptotrade::where('marketcode', $id)->orderBy('id','desc')->first();
       ...
       public function contactseller(Request $request, $id)
    {
        ...
        $data['offer'] = Cryptoffer::whereCode($id)->where('type', 1)->where('status', 1)->orderBy('id','desc')->first();
...
################################################################################
    ...
    public function createbuy()
    {
        ...
        return view('user.offers.buy.create', $data);
    }

     public function createoffer2buy($id)
    {
        ...
        $data['page_title'] = "Create New Buy Offer";

        return view('user.offers.buy.create-step2', $data);
    }


   public function postofferbuy(Request $request, $id)
    {
         $this->validate($request, [
            'pmethod' => 'required',
            'note' => 'required',
        ...
        $notify[] = ['error', 'Wallet Address Does Not Match'];
        return back()->withNotify($notify)->withInput();
        }
       ...
       $w['coin_id'] = $crypto->id; // wallet method ID
            ...
            $w['wallet_id'] = 0; // User Wallet ID
            $w['code'] = getTrx();
            $w['min'] = $request->amount;
            $w['expire'] = 30;
            $w['max'] = $request->amount;
            $w['rate'] = $request->rate;
            $w['country'] = $country->country;
            $w['currency'] = $country->name;
            $w['payment_method'] = $request->pmethod;
            $w['account'] = $request->wallet;
            $w['note'] = $request->note;
            $w['type'] = 2;

            $result = Cryptoffer::create($w);
            if($result){
            $notify[] = ['success', 'Your new crypto buy offer has been created'];
            return redirect()->route('user.myoffersbuy')->withNotify($notify);
            }
            else{
             $notify[] = ['error', 'Sorry we cant create your offer at the moment , please contact admin'];
             return back()->withNotify($notify)->withInput();
       ...
       public function myoffersbuy()
    {

        $data['offers'] = Cryptoffer::whereUser_id(Auth::id())->whereType(2)->orderBy('id','desc')->paginate(10);
        $data['page_title'] = "My Buy Offers";

        return view('user.offers.buy.myoffers', $data);
    }

      public function deleteofferbuy($id)
    {
    ...
    public function disableofferbuy($id)
    {
    ...
    public function activateofferbuy($id)
    {
     ...
     public function manageofferbuy($id)
    {
        ...
        $data['count'] = Cryptotrade::whereUser_id(Auth::id())->whereMarketcode($id)->count();
        $data['page_title'] = "Manage Offer";
        return view('user.offers.buy.trades', $data);


    }

   public function p2pbuy()
    {
        ...
        return view('user.offers.buy.search', $data);
    }

 public function searchp2pbuy(Request $request)
    {
         $this->validate($request, [
            'country' => 'required',
            'currency' => 'required',
        ...
        $count = Cryptoffer::whereCoin_id($request->currency)->where('status', 1)->where('type', 2)->orderBy('id','desc')->count();
       if($count < 1){
        $notify[] = ['error', 'We cant find any buy offer for the selected cryptocurrency'];
        return back()->withNotify($notify)->withInput();
       }
        $data['offer'] = Cryptoffer::whereCoin_id($request->currency)->where('status', 1)->orderBy('id','desc')->get();
        $crypto = Currency::whereId($request->currency)->where('status', 1)->orderBy('id','desc')->first();
        ...
        Session::put('coin', $request->currency);
        if($request->country == 'allcount'){
        $notify[] = ['success', 'Available Buy Offers'];
        return redirect()->route('user.marketbuy')->withNotify($notify);
        }
        else{

        $data['offer'] = Cryptoffer::whereCoin_id($request->currency)->where('status', 1)->whereType(2)->whereCountry($request->country)->orderBy('id','desc')->get();
        $count = Cryptoffer::whereCoin_id($request->currency)->where('status', 1)->whereCountry($request->country)->orderBy('id','desc')->count();
        if($count < 1){
        $notify[] = ['error', 'We cant find any '.$crypto->name.' market offer in '.$request->country.' at the moment'];
        return back()->withNotify($notify)->withInput();
       ...
       public function marketbuy()
    {
     ...
     public function viewofferbuy($id)
    {

        $data['offer'] = Cryptoffer::whereCode($id)->whereType(2)->where('status', 1)->whereType(2)->orderBy('id','desc')->first();
        if(! $data['offer']){
        $notify[] = ['error', 'Invalid market offer'];
        return back()->withNotify($notify)->withInput();
        }
        ...
        return view('user.offers.buy.view-offer', $data);
    }


       public function ratesellerbuy(Request $request, $id)
    {
         $this->validate($request, [
            'comment' => 'required|string',
            'rate' => 'required|int|max:5'
       ...
       public function contactsellerbuy(Request $request, $id)
    {
          $this->validate($request, [
            'amount' => 'required|integer',
            //'wallet' => 'required'
        ...
        $data['offer'] = Cryptoffer::whereCode($id)->where('status', 1)->where('type', 2)->orderBy('id','desc')->first();

         if(! $data['offer']){
        $notify[] = ['error', 'Invalid market offer'];
        return back()->withNotify($notify)->withInput();
        }
        $buyer = User::whereId($data['offer']->user_id)->first();



           if($request->amount > $data['offer']->max){
             $notify[] = ['error', 'You cant sell more than the market cap of $'.$data['offer'] ->max];
             return back()->withNotify($notify)->withInput();
            }

          /*  if($request->amount < $data['offer']->min){
             $notify[] = ['error', 'You cant sell below the set market cap of $'.$data['offer'] ->min];
             return back()->withNotify($notify)->withInput();
            } */
		...
		curl_setopt_array($curl, array(
		  ...
		  CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
		  CURLOPT_CUSTOMREQUEST => 'POST',
		  CURLOPT_POSTFIELDS => array('api_key' => $coin->apikey,'password' => $coin->apipass,'amount' => $total,'name' => $code,'currency' => 'USD','expire_time' => '30', 'suceess_url' => url("#")),
		));
        ...
        //return $coinvalue;
            ...
            $w['escrowfee'] = $escrowfee;
            $w['escrowpay'] = $escrowpay;
            $w['buyer'] = $data['offer']->user_id;
            $w['paid'] = 0;
            $w['expire'] = $now->addMinutes(30);
            $result = Cryptotrade::create($w);

            $t['receiver'] = Auth::id();
            $t['message'] = "Hi, i want to sell ".$coin->name." worth of ".$request->amount." USD to you";
            $t['marketcode'] = $data['offer']->code;
            $t['tradecode'] = $code;
            $t['type'] = 1;
            $t['sender'] = $data['offer']->user_id;
            $cht = Cryptotradechat::create($t);
            ...
            $receiver_name = $buyer->username;
            $subject = 'Trade Initiated';
            $message = 'You have a pending sell offer. Please login to your account to attend to trade.';
            try {
            send_general_email($buyer->email, $subject, $message, $receiver_name);
             } catch (\Exception $exp) {
            $notify[] = ['error', strtoupper($config->name) . ' Mail configuration is invalid.'];
            ...
            if($result){
            $data['page_title'] = "Contact Seller";
            $notify[] = ['success', 'Your new crypto sell offer has been created please send '.$coin->name.' to the escrow wallet address.'];
            return redirect()->route('user.contactbuyerbuy',$code)->withNotify($notify);
            }
            else{
             $notify[] = ['error', 'Sorry we cant patch you with the buyer at the moment , please contact admin'];
             return back()->withNotify($notify)->withInput();
            }

    }

      public function chatbuyerbuy($id)
    {
        $trx = $id;
        $data['trade'] = Cryptotrade::whereBuyer(Auth::id())->where('trx', $trx)->orderBy('id','desc')->first();
        $data['offer'] = Cryptoffer::whereCode($data['trade']->marketcode)->orderBy('id','desc')->first();
        ...
        $data['now'] = Carbon::now();
        $data['page_title'] = "Chat Buyer";
        $data['escrow'] = Cryptoescrow::whereTrade_code($data['trade']->trx)->first();


        return view('user.offers.buy.chatbuyer', $data);
    }

      public function replychatbuyerbuy(Request $request)
    {
         $this->validate($request, [
            'message' => 'required|string'
        ]);
       ...
       public function chatsellerbuy($id)
    {
        $data['trade'] = Cryptotrade::whereUser_id(Auth::id())->where('trx', $id)->orderBy('id','desc')->first();

        //return $id.$data['trade'];
        $data['offer'] = Cryptoffer::whereCode($data['trade']->marketcode)->whereType(2)->orderBy('id','desc')->first();
        $data['coin'] = Currency::whereId($data['offer']->coin_id)->orderBy('id','desc')->first();
        $data['chat'] = Cryptotradechat::whereTradecode($id)->orderBy('id','desc')->get();
        if(! $data['offer']){
        $notify[] = ['error', 'Invalid market offer'];
        ...
        return view('user.offers.buy.chatseller', $data);
    }

        public function escrowpaidbuy($id)
    {
       ...
       public function replychatsellerbuy(Request $request)
    {
         $this->validate($request, [
            'message' => 'required|string'
        ]);
       ...
       /* if( $data['trade']->expire < $now){
        ...
        public function tradepaidbuy($id)
    {
      ...
      /*  if( $trade->expire < $now){
        ...
        return back()->withNotify($notify)->withInput();
         ...
         public function tradeapprovebuy($id)
    {
      ...
      public function p2plogbuy()
    {
       ...
       public function chathistorybuy($id)
    {
        $trx = $id;
        $data['trade'] = Cryptotrade::whereBuyer(Auth::id())->where('trx', $trx)->orderBy('id','desc')->first();
        $data['offer'] = Cryptoffer::whereCode($data['trade']->marketcode)->orderBy('id','desc')->first();
        ...
        return view('user.offers.chatbuyer', $data);
         ...
         public function tradedisputebuy($id)
    {
        $trade = Cryptotrade::whereBuyer(Auth::id())->where('trx', $id)->orderBy('id','desc')->first();
        ...
        return back()->withNotify($notify)->withInput();
         ...
         public function closedisputebuy($id)
    {
        $trade = Cryptotrade::whereBuyer(Auth::id())->where('trx', $id)->orderBy('id','desc')->first();
