...
class FrontendController extends Controller
{
    ...
    public function sectionHomeContentUpdate(Request $request, $id)
    {
        ...
        return back()->withNotify($notify);
            ...
            $in['step'] = $request->start;
            $in['step1'] = $request->step1;
            $in['step2'] = $request->step2;
            $in['step3'] = $request->step3;
            $in['f1'] = $request->f1;
            $in['f11'] = $request->f11;
            $in['f2'] = $request->f2;
            $in['f3'] = $request->f3;
            ...
            $in['g7'] = $request->g7;
            $in['g8'] = $request->g8;
            $in['abouthead'] = $request->abouthead;
            $in['aboutsubhead'] = $request->aboutsubhead;
            $in['aboutbody'] = $request->aboutbody;
            $in['problemheader'] = $request->problemheader;
            $in['problemsubheader'] = $request->problemsubheader;
            $in['problem'] = $request->problem;
            $in['solution'] = $request->solution;
            $in['whitepaperheader'] = $request->whitepaperheader;
            $in['whitepapersubheader'] = $request->whitepapersubheader;
            $in['whitepaperbody'] = $request->whitepaperbody;
            $in['mobileappheader'] = $request->mobileappheader;
            $in['mobileappsubheader'] = $request->mobileappsubheader;
            $in['mobileappbody'] = $request->mobileappbody;
            $in['roadmapheader'] = $request->roadmapheader;
            $in['roadmapsubheader'] = $request->roadmapsubheader;

            if ($request->hasFile('f112')) {
                @unlink('assets/images/frontend/' . @$data->data_values->f112);
                $image = $request->file('f112');
                $filename = 'f112.'. $image->getClientOriginalExtension();;
                $location = 'assets/images/frontend/' . $filename;
                Image::make($image)->save($location);
                $in['f112'] = $filename;
            } else {
                $in['f112'] = @$data->data_values->image;
            }
            ...
            if ($request->hasFile('mobileappimage')) {
                @unlink('assets/images/frontend/' . @$data->data_values->mobileappimage);
                $image = $request->file('mobileappimage');
                $filename = 'mobileappimage.'. $image->getClientOriginalExtension();;
                $location = 'assets/images/frontend/' . $filename;
                Image::make($image)->save($location);
                $in['mobileappimage'] = $filename;
            } else {
                $in['mobileappimage'] = @$data->data_values->mobileappimage;
            }


            if ($request->hasFile('solutionimage')) {
                @unlink('assets/images/frontend/' . @$data->data_values->solutionimage);
                $image = $request->file('solutionimage');
                $filename = 'solutionimage.'. $image->getClientOriginalExtension();;
                $location = 'assets/images/frontend/' . $filename;
                Image::make($image)->save($location);
                $in['solutionimage'] = $filename;
            } else {
                $in['solutionimage'] = @$data->data_values->solutionimage;
            }



            if ($request->hasFile('problemimage')) {
                @unlink('assets/images/frontend/' . @$data->data_values->problemimage);
                $image = $request->file('problemimage');
                $filename = 'problemimage.'. $image->getClientOriginalExtension();;
                $location = 'assets/images/frontend/' . $filename;
                Image::make($image)->save($location);
                $in['problemimage'] = $filename;
            } else {
                $in['problemimage'] = @$data->data_values->problemimage;
            }



            if ($request->hasFile('aboutimage')) {
                @unlink('assets/images/frontend/' . @$data->data_values->aboutimage);
                $image = $request->file('aboutimage');
                $filename = 'aboutimage.'. $image->getClientOriginalExtension();;
                $location = 'assets/images/frontend/' . $filename;
                Image::make($image)->save($location);
                $in['aboutimage'] = $filename;
            } else {
                $in['aboutimage'] = @$data->data_values->aboutimage;
            }
